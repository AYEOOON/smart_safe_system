...
8. [개발 시 문제점 및 해결 방안](#-8-개발-시-문제점-및-해결-방안)  
9. [프로젝트 시연 동영상](#-9-프로젝트-시연-동영상)   
...
## 🔨 8. 개발 시 문제점 및 해결 방안
  **초음파 센서의 감지 불안정성**
  - **문제**
    - 사람의 위치와 환경에 따라 정확한 감지가 어려움
    - 실내 환경에서 벽, 물체 반사로 인한 예기치 않은 결과 발생

  - **해결 방안**
    - 초음파 센서를 스위치로 교체하여 사용자의 직접적인 제어로 정확하고 일관된 동작 제공
    
---

  **비밀 번호 인증 실패**
  - **문제**
    - 블루투스 연결 후 비밀번호를 입력하면 올바른 비밀번호를 입력했음에도 불구하고 인증실패 메시지가 출력되는 문제 발생

  - **해결 방안**
    - 입력된 비밀번호와 저장된 비밀번호를 한 글자씩 순차적으로 비교
```c
const char password[4] = {'1', '2', '3', '4'};

int checkPassword(const char* input) {
    for (int i = 0; i < 4; i++) { //비밀번호를 한 글자 씩 비교
        if (input[i] != password[i]) { //비밀번호 불일치 시
        return 0;
        }
    }
    return 1; //비밀번호 일치 시
